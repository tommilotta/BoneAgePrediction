...
def fit_boneage(model, train_gen, train_steps, val_gen, val_steps, epochs=1, callbacks=None, path='./', gender=True):
    ...
    print('/n TRAINING FINISHED /n')
    print('Training time: {} /n'.format(model_time))
    print('Loss: {} /n'.format(model_history.history['loss']))
    print('MAE in months: {} /n'.format(model_history.history['mae_in_months']))
    print('Parameters: {} /n'.format(model.count_params()))
